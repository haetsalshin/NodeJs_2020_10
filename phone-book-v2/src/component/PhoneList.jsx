...
const PhoneList = ({ phoneBooks, deletePhoneBooks, editableBooks }) => {
  ...
  /**
   * tr tag를 클릭 했을 때 사용할 event 핸들러
   * tr tag를 클릭을 했지만 우리가 사용할 것은 td tag가 목적이므로
   * e.targer.className 속성을 사용하여 어떤 td가 클릭되는지 확인하자
   */
  const trOnClick = (e) => {
    ...
    if (className === "delete") {
      if (window.confirm(name + "을 정말로 삭제할까유?!")) {
        //alert(name + "데이터 삭제");
        deletePhoneBooks(id);
        return false;
      }
  ...
  /**
   * map을 사용하여 배열요소로 화면을 만들 때
   * PK에 해당하는 key 라는 요소를 반드시 지정해야 한다
   * 그렇지 않으면 경고가 나타나고
   * 데이터가 많을 때 화면이 출렁거리는 현상이 발생한다
