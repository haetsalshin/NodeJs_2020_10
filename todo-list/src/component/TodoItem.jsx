  ...
  render() {
    ...
    const onClick = (e, id) => {
      if (e.target.className === "delete-item") {
        if (window.confirm("삭제할까요?( •̀.̫•́)✧")) {
          // alert("삭제됨ŏ̥̥̥̥םŏ̥̥̥̥");
          onDeleteItem(id);
          return false;
        }
      } else {
    ...
    // {todo.inCompelte && <div><.div}
    // {} 로 묶인 부분은 내부의 연산결과에 따라 rendering을 수행하는 코드
    // todo.inComplete 값이 true일 경우만 <div></div>를 rendering하여 화면에 보이라는
    // 조건부 rendering
  }
}
